...
Enterprise job monitoring for Databricks with optional Lakebase acceleration.
...
- System tables enabled (`system.lakeflow`, `system.access`)
- Node.js 18+ and Python 3.10+
- A SQL Warehouse (serverless recommended)
- *(Optional)* A Lakebase instance for sub-100ms query performance
...
The app reads from materialized views, not raw system tables directly. A Spark Declarative Pipeline (SDP) ingests from `system.lakeflow.*` and `system.access.*` into your catalog/schema.
...
-- You should see: job_runs_latest, jobs_latest, synced_workspaces, etc.
```
...
| `lakebase_instance_name` | No        | Enables sub-100ms queries via Lakebase          |
